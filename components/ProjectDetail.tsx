...
export const ProjectDetail: React.FC<ProjectDetailProps> = ({ project, currentUser, onUpdateProject, onBack, onDeleteProject, isGuestView = false, existingTags = [] }) => {
  ...
  const handleAddFile = () => {
      ...
      if (!name) return;
      const url = prompt("Google Drive link:", "https://drive.google.com/...");
      
      const newFile: ProjectFile = {
          id: `file_${Date.now()}`,
          name,
          url,
          description: "New file added by user.",
          type: 'document',
      ...
      <header className="bg-white p-4 border-b border-slate-200 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
           ...
           <div className="flex items-center">
                ...
                {!isGuestView && (
                    <button onClick={() => alert('Manage collaborators')} className="p-2 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg"><Users className="w-4 h-4"/></button>
                )}
           </div>
